...
<b>02 July 2021</b>: (1) Page 41 of the Manual was updated to inform the user how to pause and step through the CELL application. (2) Directory name changed from "CPLD Files" to "CPLD_Files" because CUPL doesn't recognize spaces in directory names. (3) The tabulations of all three .PLD files have been re-formatted for better readability. (4) The buried logic nodes are now left unassigned to pins in CAVIA.PLD and SKUNK.PLD as well, not only in SPACER.PLD. This way, the compiler makes the assignments automatically. The functionality doesn't change at all, but the .PLD files are easier to interpret this way.
<P>
